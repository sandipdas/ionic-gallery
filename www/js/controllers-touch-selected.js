...
function AppCtrl($rootScope, $scope, $ionicModal, $timeout,$state, $ionicSideMenuDelegate, FavoriteService) {
    ...
    // ---------slider---------
        function setAsWallpaper(imagePath) {
            ...
            // setTimeout(function(){
            //     $rootScope.$broadcast('setwallpaper-success');
            // }, 1000);
            // return;
            // For saving image
    ...
    function touchPlay(e){
        ...
        lastElement = ele.$$hashKey;
        selectImage(ele);

        ppp = debounce(play, 50);

        document.getElementById('brick-container').addEventListener('touchmove', ppp , false);
        document.getElementById('brick-container').addEventListener('touchstart', startTouch , false);

 
        function play(event){
            // event.cancelable=false;
            // console.log(new Date());
            if (!event.targetTouches[0].clientX || !event.targetTouches[0].clientY) {
                return;
            }
            var secondElement = document.elementFromPoint(event.targetTouches[0].clientX, event.targetTouches[0].clientY);
            if (!secondElement) {
                return;
            }
            secondElement = secondElement.parentElement;
            // lastElement = secondElement.$$hashKey;
            if (lastElement == null || lastElement != secondElement.$$hashKey) {
                // secondElement.classList.toggle('selected');
                selectImage(secondElement)
            }
    ...
    $timeout(function () {
        document.getElementById('brick-container').addEventListener('touchend', function (event) {
            if (!playStart) {
                return;
            }
            playStart = false;
            $ionicSideMenuDelegate.canDragContent(true);
            document.getElementById('brick-container').removeEventListener('touchmove', ppp);
            document.getElementById('brick-container').removeEventListener('touchstart', startTouch);
            // stopTouchPlay();
        } , false);
    }, 1000);
    

    function startTouch(event1) {
        playStart = true;
    ...
    function stopTouchPlay() {
        $ionicSideMenuDelegate.canDragContent(true);
        $rootScope.isSelectSate = false;
        document.getElementById('brick-container').removeEventListener(event, ppp);
    }
